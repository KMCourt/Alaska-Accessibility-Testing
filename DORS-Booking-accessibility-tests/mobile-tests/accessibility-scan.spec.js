/**
 * ===========================================
 * ACCESSIBILITY SCAN TEST — DORS-Booking MOBILE
 * ===========================================
 * Runs axe-core accessibility scans against all pages
 * in the DORS Booking site on emulated mobile devices:
 ...
 *
 * HOW TO RUN:
 * All devices:  npx playwright test --config=dors-booking.config.js --project="iPhone 15 Pro" --project="Galaxy S24" --project="Pixel 7"
 * One device:   npx playwright test --config=dors-booking.config.js --project="iPhone 15 Pro"
 * ===========================================
 */
...
const resultsDir = path.join(__dirname, '..', 'DORS-Booking-results', today, 'mobile');
...
test.describe('Accessibility Scan — Mobile', () => {
  ...
  for (const pageDef of PAGES) {
    test(`Scan: ${pageDef.name}`, async ({ page }, testInfo) => {
      ...
      const screenshotFile = `${safeName}_${projectName.replace(/\s+/g, '_')}.png`;
        ...
        for (let i = 0; i < violation.nodes.length; i++) {
          ...
          if (target) {
            try {
              ...
              const shotFile = `${safeName}_${projectName.replace(/\s+/g, '_')}_${violation.id}_${i}.png`;
      ...
      const jsonPath = path.join(jsonDir, `${safeName}_${projectName.replace(/\s+/g, '_')}.json`);
