...
/**
 * Waits for an OTP email to arrive and returns the 6-digit code.
 *
 * @param {object} options
 * @param {string} options.apiKey    - Testmail API key (from .env)
 * @param {string} options.namespace - Testmail namespace e.g. 3ejw1
 * @param {string} options.tag       - Email tag e.g. test
 * @param {number} options.after     - Timestamp (ms) — only fetch emails received after this time
...
async function getOtp({ apiKey, namespace, tag, after, maxWaitMs = 60000 }) {
  ...
  while (Date.now() < deadline) {
    // Wait before polling to give the email time to arrive
    ...
    // Prefer the OTP passcode email over other emails (e.g. booking confirmations)
    const otpEmail = result.emails.find(e =>
      e.subject && e.subject.toLowerCase().includes('passcode')
    ) || result.emails[0];

    // Strip HTML tags and search for a 6-digit code
