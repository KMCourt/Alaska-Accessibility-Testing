/**
 * ===========================================
 * GLOBAL TEARDOWN — DORS-Booking
 * ===========================================
 * Runs once after ALL tests across ALL browsers/devices complete.
 * Reads every JSON result file written during the scan,
 * builds separate browser and mobile HTML reports,
 * and notifies Teams.
 * ===========================================
 */
...
module.exports = async function globalTeardown() {
  ...
  const baseResultsDir = path.join(__dirname, '..', 'DORS-Booking-accessibility-tests', 'DORS-Booking-results');
  ...
  if (browserResults.length > 0) {
    ...
    console.log(`\n✅ Browser report: DORS-Booking-accessibility-tests/DORS-Booking-results/${today}/browser/report.html`);
  }
  ...
  if (mobileResults.length > 0) {
    ...
    console.log(`✅ Mobile report:  DORS-Booking-accessibility-tests/DORS-Booking-results/${today}/mobile/report.html`);
  }
  ...
  if (browserResults.length > 0) printTotals('DORS-BOOKING BROWSER (Chrome / Firefox / Edge)', browserResults);
  if (mobileResults.length  > 0) printTotals('DORS-BOOKING MOBILE  (iPhone / Galaxy / Pixel)',  mobileResults);
  ...
  if (browserResults.length > 0) {
    await postToTeams({
      ...
      today,
      regressions,
      label: 'DORS-Booking Browser Scan (Chrome / Firefox / Edge)',
  ...
  if (mobileResults.length > 0) {
    await postToTeams({
      ...
      today,
      regressions,
      label: 'DORS-Booking Mobile Scan (iPhone / Galaxy / Pixel)',
