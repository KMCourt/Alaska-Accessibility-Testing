...
module.exports = async function globalTeardown() {
  ...
  const baseResultsDir = path.join(__dirname, '..', 'Trainer-Test', 'trainer-results');
  ...
  if (browserResults.length > 0) {
    ...
    console.log(`\n✅ Browser report: Trainer-Test/trainer-results/${today}/browser/report.html`);
  }
  ...
  if (mobileResults.length > 0) {
    ...
    console.log(`✅ Mobile report:  Trainer-Test/trainer-results/${today}/mobile/report.html`);
  }
  ...
  if (browserResults.length > 0) printTotals('TRAINER BROWSER (Chrome / Firefox / Edge)', browserResults);
  if (mobileResults.length  > 0) printTotals('TRAINER MOBILE  (iPhone / Galaxy / Pixel)',  mobileResults);
  ...
  if (browserResults.length > 0) {
    await postToTeams({
      ...
      today,
      regressions,
      label: 'Trainer Browser Scan (Chrome / Firefox / Edge)',
  ...
  if (mobileResults.length > 0) {
    await postToTeams({
      ...
      today,
      regressions,
      label: 'Trainer Mobile Scan (iPhone / Galaxy / Pixel)',
