/**
 * ===========================================
 * ACCESSIBILITY SCAN TEST — ConstBA
 * ===========================================
 * Runs axe-core accessibility scans against all pages
 * in the Construction Booking site, in Chrome, Firefox and Edge.
 *
 * Generates ONE consolidated HTML report at:
 *   ConstBA-results/YYYY-MM-DD/report.html
 *
...
const PAGES = [
  ...
  // -----------------------------------------------
  {
    ...
    setup: async (page) => {
      ...
      // Open the user menu and click "View booked courses"
  ...
  },
  {
    ...
    setup: async (page) => {
      ...
      // Open the user menu and click "User profile"
      await page.locator('[aria-haspopup="true"]').click();
      await page.waitForSelector('[role="menu"]', { state: 'visible' });
...
test.describe('Accessibility Scan', () => {
  ...
  for (const pageDef of PAGES) {
    test(`Scan: ${pageDef.name}`, async ({ page }, testInfo) => {
      ...
      // Use per-page timeout if specified
      ...
      // Capture the actual URL after any navigation
...
// Report generation and Teams notification are handled by utils/constba-global-teardown.js
